...
import { listNotes } from '../redux/reducer';


class NoteEntries extends React.Component {
    constructor(props) {
        super(props);
        // console.log(props)
        this.state = { 
  ...
  componentDidMount() {
    ...
    this.props.listNotes().then(notes => {
      this.filterNotes()
    });
 }
 ...
 componentDidUpdate(prevProps) {
  ...
  filterNotes() {
    var todayNotes = this.props.notes.filter(f => f.dateTime >= this.state.date.startOf('day') && f.dateTime < this.state.date.endOf('day'))
    this.setState({todaysEntries: todayNotes})
  }
 ...
 _setNavigationParams() {
  ...
  renderListItem(item) {
    return (
      <View style={styles.listItem}> 
        <Text style={[styles.time, {textAlignVertical: 'top'}]}>{moment(item.dateTime).format('LT')}</Text> 
  ...
  renderActionButton() {
    return ( 
        <ActionButton buttonColor={Colors.teal} onPress={() => this.props.navigation.navigate('AddNote') }></ActionButton>
    );
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(NoteEntries);